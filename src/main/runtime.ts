...
export class AppRuntime {
  ...
  async init(): Promise<void> {
    ...
    this.world.on('state', (worldState) => {
      const musical = this.music.update(worldState);
  ...
  async applyConfigText(text: string): Promise<{ ok: boolean; errors?: string[] }> {
    ...
    try {
      await this.configService.save(result.config);
      this.config = result.config;
      this.world.applyConfig(this.config);
      this.music.applyConfig(this.config);
      this.resolveRemovedLayers(previous, this.config);
      this.emitSnapshot();
      return { ok: true };
    } catch (error) {
      this.config = previous;
      return { ok: false, errors: [error instanceof Error ? error.message : 'Failed to apply config'] };
    }
  }
